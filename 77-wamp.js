...
//EM    var fs = require('fs');

//Must be active when running from Linux
//EM	var fetch = require("node-fetch");
	
//EM
	process.env.NODE_TLS_REJECT_UNAUTHORIZED = "0"

//EM	const axios = require("axios");
...
//EM	
    ...
    function WampClientNode(config) {
		...
		RED.log.info("WampClientNode invoked.");
    ...
    function WampClientOutNode(config) {
		...
		RED.log.info("WampClientOutNode invoked.");
    ...
    function WampClientInNode(config) {
		...
		RED.log.info("WampClientInNode invoked.");
    ...
    function WampClientCallNode(config) {
		...
		RED.log.info("WampClientCallNode invoked.");
	...
	async function GetToken(id, passw, address)
	{
		...
		RED.log.info("WampClientPool: authid: " + id + ". password: " + passw + ". url: " + address);
		...
		RED.log.info("WampClientPool: addrPortArray: " + addrPortArray);


		RED.log.info("WampClientPool: IP-addr: " + ipAddr + ". Encrypted: " + encrypt.toString());
	...
	{
		RED.log.info("WampClientPool invoked.");
                ...
                if (!connections[uri])
				{
                    connections[uri] = (function ()
					{
                        ...
                        var setupWampClient = function () {
							...
							RED.log.info("WampClientPool: authid: " + authid + ". password: " + password + ". url: " + address);
